...
def cross2dot_b(v: torch.Tensor):
    n_b,_ = v.shape
    vx=v[:,0].view(-1,1)
    vy=v[:,1].view(-1,1)
    vz=v[:,2].view(-1,1)
    zeros = torch.zeros(n_b).to(v.device).view(-1,1)
...
def get_wrench(contact_in_ankle_frame):
    ...
    G_lin = torch.eye(3).to(contact_in_ankle_frame.device).view(1,3,3)
...
def get_contact_jacobis6D(model, q, ids, device="cuda"):
...
def get_contact_wrench(
    ...
    qs_np = qs.clone().cpu().detach().numpy().astype(float)

    l_toe_in_ankle = torch.FloatTensor(
        [get_contact_point_in_ankle_frame(
            model, q, rbdl_dic, "left_toe", "left_ankle") for q in qs_np]).to(device)
    l_heel_in_ankle= torch.FloatTensor(
        [get_contact_point_in_ankle_frame(
            model, q, rbdl_dic, "left_heel", "left_ankle") for q in qs_np]).to(device)
    r_toe_in_ankle = torch.FloatTensor(
        [get_contact_point_in_ankle_frame(
            model, q, rbdl_dic, "right_toe", "right_ankle") for q in qs_np]).to(device)
    r_heel_in_ankle= torch.FloatTensor(
        [get_contact_point_in_ankle_frame(
            model, q, rbdl_dic, "right_heel", "right_ankle") for q in qs_np]).to(device)
